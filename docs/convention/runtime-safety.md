...
- **A failure that reaches no one is the defect.** A `try { ... } catch { /* ignore */ }` is one shape of it. A raised domain error that no handler maps is the other: it reaches the caller as a generic 500 and the reader as nothing.
