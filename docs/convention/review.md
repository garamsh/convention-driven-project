...
- **Confirm the checks belong to the head commit**, not merely that checks are green. Compare the PR's head ref with the commit the check suite ran on: green on an older commit says nothing about the head. A head commit with no checks is not a pending state to wait through — it is an unverified change, reported rather than merged.
