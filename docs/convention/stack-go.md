...
- `internal/<domain>/<responsibility>.go` — the service's method
...
- **Every dependency interface gets its own file**, named for the
  ...
  contract.
- **Its implementations go in `internal/<domain>/<pkg>/` where a
  neighbouring package can construct the aggregate**, so a driver
  import (`pgx`, a vendor SDK) stays out of the domain package's
  import graph and its tests; where unexported fields close the
  aggregate, only its own package can construct one, so the
...
- `internal/<domain>/<domain>.go`, `internal/<domain>/service.go` and
  ...
  against.
- `internal/<domain>/<responsibility>.go` and the implementations in
