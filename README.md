...
1. **Pick the conventions that apply.** Under `docs/convention/`, keep the `stack-*.md` files matching the project's stack and the `arch-*.md` file matching its structure, plus the base of any kept file the conventions index gives one; delete the rest. Keeping none of either is a valid outcome — a project neither covers runs on the stack-neutral files alone. Done when every remaining `stack-*.md` and `arch-*.md` names something the project uses.
