...
4. **Choose the system's structure.** Read the candidates under `docs/architecture/structures/` side by side and take one. Record the choice as an ADR, carrying that file's §Signs the choice was wrong into the ADR's §Signs the decision was wrong and its §Boundaries and dependency direction into `docs/architecture/structure.md` (`docs/architecture/README.md` rules 6 and 7). Delete `docs/architecture/structures/` only once both exist — it is what they are written from. Done when an ADR names the shape taken and carries every bullet of that shape's §Signs the choice was wrong, `structure.md` carries every bullet of its §Boundaries and dependency direction, and `docs/architecture/structures/` is gone.
